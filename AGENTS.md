...
- Consumer registration is open and does not require an invite code. `invite_codes` and `invite_redemptions` remain service-role-only tables; browser code must never read them directly or call public Auth signup or Admin APIs, and every registration must use the FastAPI registration endpoint so server-side rate limiting, consent recording, and account creation are enforced. Invite codes remain an optional source: when supplied, they are validated and consumed; when absent, registration follows the same rate-limit and consent path. Existing accounts, administrators, and organization members remain outside this gate.
...
- Resolved: published statistics use the mandatory shared provenance contract in `backend/app/services/provenance.py`; `tests/unit/test_provenance_response_contract.py` and `tests/api/test_provenance_endpoint_contract.py` enforce its required fields and the latter covers `/api/org/region-stats` and `/api/analysis`. Reports and exports serialize the same fields, while `20260920000400_dataset_provenance_contract.sql` supplies forward-only MLIT/e-Stat storage columns and a precise missing-field write trigger.
- The analysis page now reads structured location/asset fields with a single tested legacy-title parser; every current content-library record carries prefecture/city/ward. The content library still lacks a multi-month trend dataset, but authorized MLIT region statistics now expose the quota-metered `/api/org/region-stats/trend` sequence with per-period provenance; `tests/integration/test_region_stats_postgres.py::test_real_postgres_trend_response_is_chronological_provenance_complete_and_metered` supplies local synthetic-fixture evidence.
...
- Automated coverage and CI gates now exist, but are not a production-readiness claim: `.github/workflows/release-gate.yml` has Python (`python-pytest`, `python-compileall`, `pip-check`), Node, Playwright, disposable SQL/RLS, supply-chain, repository-policy, and release-evidence jobs; each required result is enforced by `scripts/ci/release_evidence.py`, while `scripts/ci/check_release_policy.py` checks repository policy. The current local full-suite evidence is `734 passed, 1 skipped` using `DATABASE_URL=... TEST_DATABASE_URL=... PYTHONPATH=. backend/.venv/bin/python -m pytest -q`; rerun it for current evidence rather than treating this count as permanent. A repeatable local logical export/isolated-restore drill is `scripts/restore_drill.py`, and the read-only production check is `deploy/observability-check.sh`. Still missing: CI/release evidence is not a substitute for current production access/alert delivery/on-call response evidence; production backup availability and an approved, non-peak restore exercise remain separately required; no claim of automated production alert routing, SLO monitoring, or tested incident response is made here.
