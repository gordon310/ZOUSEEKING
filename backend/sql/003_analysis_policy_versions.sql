...
create or replace function public.prevent_policy_version_overlap()
returns trigger
language plpgsql
as $$
begin
  if exists (
    ...
    where p.policy_key = new.policy_key
      and p.id <> coalesce(new.id, '00000000-0000-0000-0000-000000000000')::uuid
      and daterange(p.effective_from, coalesce(p.effective_to + 1, '9999-12-31'::date), '[)')
          && daterange(new.effective_from, coalesce(new.effective_to + 1, '9999-12-31'::date), '[)')
  ) then
...
create trigger prevent_policy_version_overlap
before insert or update on public.policy_documents
for each row execute function public.prevent_policy_version_overlap();

create or replace function public.prevent_published_metric_update()
returns trigger
language plpgsql
as $$
begin
  if not public.is_service_role() then
    raise exception 'analysis metric history is immutable';
  end if;
  return new;
end;
$$;
...
for each row execute function public.prevent_published_metric_update();
