...
## 7c. 多 Agent 并发协调规则（2026-09-05 用户确认，防会话冲突）

- **单写者原则**：同一 checkout（主工作目录 JPPropDIs）同一时间只允许一个 agent 写。Hermes/自主班在主工作目录作业时，ChatGPT Codex/CLI 会话一律**基于 origin/main 另开分支或 worktree**，不在主目录并发提交。
- **任务登记**：用户派给 ChatGPT Codex 的任务，先把文件/范围告诉 Hermes 登记（避免与当前单元重叠）；Hermes 派单元前也检查是否有 codex 会话在改同文件（`ps` 查 codex exec + `git status` 脏树即视为占用，改等）。
- **脏树即占用**：工作区有未提交改动时，任何自主班/单元不得叠加作业（进行中防抖），只能先审查落定。
- **DB/staging 写统一走 Hermes 确认线**：Codex 会话只做代码与本地验证，不直接写 staging/production DB、不 repair/push migration（避免双重批准与 history 冲突）。
- **基线纪律**：任何会话开工前 `git fetch` 并确认基于 origin/main 最新（分叉历史已于 09-04 解决，禁止再从旧快照 worktree 干活）。
