...
## 2b. D6 决策记录(2026-09-07 用户确认:**D6a**)

深度报告 = 国交省成交侧(sale,政府开放数据标注出处)+ **租金侧标注"待授权源"**(不输出 SUUMO rents,不显示假租金;租买比类指标在租金不可比时明确标注不可比/隐藏)。SUUMO 商务许可(用户动作)可并行,拿到后再开租金侧,不阻塞 P2 开发。P2-3b 报告引擎首版只消费 `sale` 数值字段;`rents` 字段保留在数据层(未来授权后启用),报告输出层不消费。
