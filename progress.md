...
- **验证证据(全部本机实跑)**:`pytest tests/smoke/test_staging_synthetic_smoke.py -q` → **15 passed**;`pytest tests/unit tests/architecture -q` → **482 passed / 91 skipped**(与 09-24 下午基线一致,零回归);全量无库 `pytest -q` → **694 passed / 113 skipped / 0 failed**(基线 679,新增 15 即本批用例);`compileall -q scripts tests/smoke` OK;`git diff --check` 净;CLI 实测:`--plan` exit 0、生产 host exit 2、缺 `--allow-staging` exit 2、缺凭据 exit 2、非 hex commit exit 2。**CI:Release Gate `d3d47d8` run `36004000092` success(七 job 全绿:Playwright / Node / Disposable SQL+RLS / Repository policy / Supply-chain / Python / Release evidence)**。
...
## 回滚演练完成 + D-12 入生产(2026-09-25 下午班,本 BOT)

- **授权**:用户选 B —— 回滚演练**前滚目标改为 `main`**,即顺带把 D-12(开放注册)上线。清单在执行前已逐条列出并经用户批准。
- **演练时间线(生产 UTC 实测,非估算)**:
  | T | 时间 | 事件 |
  |---|---|---|
  | T0–T1 | 15:18:44 | 基线记录 + `git checkout fd13974`(回滚目标 = D-13 部署前生产版本) |
  | T2 | 15:19:06 | 重建并替换容器完成 —— **22 秒** |
  | T3–T4 | 15:19:52 | `fetch` + `checkout main` + `pull --ff-only` → `711c73f` |
  | T5 | 15:20:30 | 重建并替换容器完成 —— **38 秒** |
  | T6 | 15:20:41 | 前滚后验证完成 |
- **停机口径**:compose 先 build 再替换,构建期间旧容器仍在服务 → **不可用窗口仅为容器替换的瞬间(秒级),不是构建时长(22/38 秒)**;nginx 全程未重启(静态资源由挂载即时生效)。
- **验证证据(两次替换后各一轮,全部实跑)**:
  - 回滚后:`/health/ready` **200**、`zoubeacon.app` **200**、未鉴权 `/api/admin/overview` **401**、4 容器 Up、前端版本回到 **`20260917-r61`**(与 fd13974 时代一致)。
  - 前滚后:同上三项通过,前端版本 **`20260925-r64`**。
  - **D-12 生效证明(关键)**:`POST /api/auth/invite-register` 带**无 `invite_code`** + 无效邮箱 → **`http=400 {"code":"invite_registration_invalid"}`**;旧版本同一请求为 **422**(缺必填字段)。探针用无效地址、**未创建任何账号**;仅消耗 1 次 `consumer_registration` 共享限流计数(该端点的正常行为)。
- **兼容性前置判断(演练前已完成)**:`fd13974` 之后的迁移**只新增表**(邀请表、共享限流表)与**数据回填**(provenance),旧代码不引用它们 → 旧代码 + 当前 54 迁移 schema 兼容,故回滚**不需要**动数据库。全程**零 DB 结构/数据变更**。
- **遗留风险(已写入演练文档,不粉饰)**:① **数据库级恢复仍未演练**(provider PITR/隔离恢复属 C04,未批准未演练);② **在飞报告任务 / 支付 webhook 处理中的回滚未演练**(outbox 租约设计预期可自愈,但未在真实回滚下验证);③ 含**破坏性迁移**的版本回滚需 expand/backfill/switch/contract 另一套流程,本次未覆盖;④ 替换窗口内请求直接失败,无排队或维护页。
