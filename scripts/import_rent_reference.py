...
IMPORT_COLUMNS = ("source_key", "source_label", "prefecture", "city", "ward", "geo_level", "scope_label", "rent_jpy_per_sqm_month", "rent_jpy_per_sqm_month_excl_zero", "survey_year", "survey_label", "observed_month", "source_url", "license_label", "fetched_at")
...
async def upsert_rows(database_url: str, rows: list[dict[str, Any]]) -> dict[str, int]:
    ...
    try:
        ...
        updated = await conn.fetchval("""with changed as (update public.rent_reference_stats target set source_label=source.source_label, rent_jpy_per_sqm_month=source.rent_jpy_per_sqm_month, rent_jpy_per_sqm_month_excl_zero=source.rent_jpy_per_sqm_month_excl_zero, survey_label=source.survey_label, source_url=source.source_url, license_label=source.license_label, fetched_at=source.fetched_at from _rent_reference_import source where target.source_key=source.source_key and target.prefecture=source.prefecture and target.city=source.city and target.ward=source.ward and target.scope_label=source.scope_label and target.survey_year=source.survey_year and target.observed_month is not distinct from source.observed_month and (target.source_label, target.rent_jpy_per_sqm_month, target.rent_jpy_per_sqm_month_excl_zero, target.survey_label, target.source_url, target.license_label) is distinct from (source.source_label, source.rent_jpy_per_sqm_month, source.rent_jpy_per_sqm_month_excl_zero, source.survey_label, source.source_url, source.license_label) returning 1) select count(*) from changed""")
        inserted = await conn.fetchval("""with added as (insert into public.rent_reference_stats (source_key,source_label,prefecture,city,ward,geo_level,scope_label,rent_jpy_per_sqm_month,rent_jpy_per_sqm_month_excl_zero,survey_year,survey_label,observed_month,source_url,license_label,fetched_at) select source_key,source_label,prefecture,city,ward,geo_level,scope_label,rent_jpy_per_sqm_month,rent_jpy_per_sqm_month_excl_zero,survey_year,survey_label,observed_month,source_url,license_label,fetched_at from _rent_reference_import source where not exists (select 1 from public.rent_reference_stats target where target.source_key=source.source_key and target.prefecture=source.prefecture and target.city=source.city and target.ward=source.ward and target.scope_label=source.scope_label and target.survey_year=source.survey_year and target.observed_month is not distinct from source.observed_month) returning 1) select count(*) from added""")
...
def parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--source", choices=("estate_housing_land", "kouri"), default=None)
...
def main(argv: list[str] | None = None) -> int:
    ...
    selected = ["estate_housing_land", "kouri"] if args.all or not args.source else [args.source]
    files = list(args.file or [])
    rows: list[dict[str, Any]] = []
    total = {"skipped": 0, "unmapped_prefecture": 0, "unmapped_city": 0}
    for source in selected:
        path = files.pop(0) if files else _download(HOUSING_URL if source == "estate_housing_land" else KOURI_URL, Path("/tmp") / f"zouseeking-{source}.xlsx")
        parsed, report = parse_housing_land_workbook(path) if source == "estate_housing_land" else parse_kouri_workbook(path)
        rows.extend(parsed)
