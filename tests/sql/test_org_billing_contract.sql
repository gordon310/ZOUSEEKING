...
begin
  ...
  from (values
    ...
    ('exports','id'), ('exports','owner_user_id'), ('exports','organization_id'), ('exports','idempotency_key'), ('exports','status'), ('exports','row_count'), ('exports','csv_content'), ('exports','created_at'),
  ...
  if not exists (select 1 from pg_indexes where schemaname='public' and tablename='exports' and indexname='uq_exports_org_owner_idempotency') then raise exception 'missing organization export idempotency index'; end if;
  raise notice 'test_org_billing_contract: all assertions passed (columns=74, usage=1, orders=1, subscriptions=1, idempotency_index=1, rollback=yes)';
end $$;
