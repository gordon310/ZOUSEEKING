...
test("reads /api/admin/* with a Bearer token when configured; 403 degrades visibly", async ({ page }) => {
    ...
    await page.route("https://admin-backend.test/api/admin/**", async (route) => {
      ...
      if (url.pathname === "/api/admin/service/tasks") {
        // service ledger: member_ops can view; return an empty real ledger.
        await route.fulfill({
          status: 200,
          contentType: "application/json",
          body: JSON.stringify({ total: 0, page: 1, page_size: 50, items: [] }),
        });
        return;
      }
