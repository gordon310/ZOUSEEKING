...
function renderRegionStats() {
  ...
  const disclosure = data.disclosure?.code === "tower_merged_into_apartment"
    ...
    : "";
  const rent = data.rent_reference;
  const rentRatio = data.rent_to_price_ratio;
  const rentYen = rent ? Math.round(Number(rent.rent_jpy_per_sqm_month)) : 0;
  const annualRent = rentYen * 12;
  const meanPriceYen = Math.round(Number(data.mean_unit_price_jpy_per_sqm));
  const rentGeoLevelLabel = rent?.geo_level_label || rentRatio?.geo_level_label || "";
  const rentMethod = rentRatio?.geo_level === "prefecture"
    ? formatUiText("regionStats.rentMethodFallback", "按所在{geoLevelLabel}口径（该市区町村无官方数据）；不含管理费/修缮费/空置等费用", { geoLevelLabel: rentGeoLevelLabel })
    : formatUiText("regionStats.rentMethod", "按官方家賃（{survey}・{scope}）与本市㎡均价计算；不含管理费/修缮费/空置等费用", { survey: rent?.survey_label, scope: rent?.scope_label, geoLevelLabel: rentGeoLevelLabel });
  const rentText = rent && rentRatio
    ? `<div class="stat-card rent-detail-card"><strong>${escapeHtml(formatUiText("regionStats.ratioGross", "租售比(毛) 约 {value}%", { value: (Number(rentRatio.gross_value) * 100).toFixed(1) }))}</strong><small>${escapeHtml(formatUiText("regionStats.rentDetailRent", "官方家賃 {rent} 円/㎡/月（{geoLevelLabel}・{survey}）", { rent: rentYen.toLocaleString("en-US"), geoLevelLabel: rentGeoLevelLabel, survey: rent.survey_label }))}</small><small>${escapeHtml(formatUiText("regionStats.rentDetailAnnual", "年家賃 {annualRent} 円/㎡/年（{rent} × 12）", { annualRent: annualRent.toLocaleString("en-US"), rent: rentYen.toLocaleString("en-US") }))}</small><small>${escapeHtml(formatUiText("regionStats.rentDetailPrice", "㎡均价 {meanPrice} 円/㎡（官方成交均值・{sampleSize} 笔）", { meanPrice: meanPriceYen.toLocaleString("en-US"), sampleSize: data.sample_size }))}</small><small>${escapeHtml(formatUiText("regionStats.rentDetailFormula", "计算式 {annualRent} ÷ {meanPrice} = {value}%", { annualRent: annualRent.toLocaleString("en-US"), meanPrice: meanPriceYen.toLocaleString("en-US"), value: (Number(rentRatio.gross_value) * 100).toFixed(1) }))}</small><small>${escapeHtml(rentMethod)}</small>${data.monthly_rent_reference ? `<small>${escapeHtml(formatUiText("regionStats.monthlyRent", "官方家賃月度动向：{rent} 円/㎡/月（{month}・都市别口径）", { rent: Math.round(Number(data.monthly_rent_reference.rent_jpy_per_sqm_month)).toLocaleString("en-US"), month: data.monthly_rent_reference.observed_month }))}</small>` : ""}<small>${escapeHtml(formatUiText("regionStats.rentDetailNote", "说明 毛回报；不含管理费/修缮费/空置等费用；家賃基准为 {survey} 官方调查", { survey: rent.survey_label }))}</small><small>${escapeHtml(uiText("regionStats.rentSource", "出典:政府統計の総合窓口(e-Stat)(https://www.e-stat.go.jp/)(加工して作成)"))}</small></div>`
