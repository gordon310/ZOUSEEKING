...
function renderRegionStats() {
  ...
  const rentText = rent && rentRatio
    ? `<p>${escapeHtml(formatUiText("regionStats.ratioGross", "租售比(毛) 约 {value}%", { value: (Number(rentRatio.gross_value) * 100).toFixed(1) }))}</p><small>${escapeHtml(formatUiText(rentRatio.geo_level === "prefecture" ? "regionStats.rentMethodFallback" : "regionStats.rentMethod", "按官方家賃（{survey}・{scope}）与本市㎡均价计算；不含管理费/修缮费/空置等费用", { survey: rent.survey_label, scope: rent.scope_label, geoLevelLabel: rent.geo_level_label }))}</small>${data.monthly_rent_reference ? `<small>${escapeHtml(formatUiText("regionStats.monthlyRent", "官方家賃月度动向：{rent} 円/㎡/月（{month}・都市别口径）", { rent: Math.round(Number(data.monthly_rent_reference.rent_jpy_per_sqm_month)).toLocaleString("en-US"), month: data.monthly_rent_reference.observed_month }))}</small>` : ""}`
    : `<p>${escapeHtml(uiText("regionStats.ratio", "租售比：暂不可用（租金数据未授权）"))}</p>`;
  result.innerHTML = `${disclosure}<p class="stats-context">${escapeHtml(context)}</p><div class="stats-summary"><strong>${escapeHtml(uiText("regionStats.mean", "均价"))} ${escapeHtml(uiText("regionStats.approx", "约"))} ${escapeHtml(tenThousands(data.mean_unit_price_jpy_per_sqm))} 万円/㎡</strong><small>${escapeHtml(uiText("regionStats.medianValue", "中位数"))} ${escapeHtml(uiText("regionStats.approx", "约"))} ${escapeHtml(tenThousands(data.median_unit_price_jpy_per_sqm))} 万円/㎡</small><small>${escapeHtml(uiText("regionStats.meanVsMedian", "均价 = 全部成交的平均值,高价房源会把均价拉高;中位数 = 成交价排序后取中间,更能代表典型行情。"))}</small></div><div class="stat-grid"><div class="stat-card"><strong>${escapeHtml(uiText("regionStats.quartiles", "主力成交价带"))} ${escapeHtml(tenThousands(data.p25))} 〜 ${escapeHtml(tenThousands(data.p75))} 万円/㎡</strong><small>${escapeHtml(uiText("regionStats.middleHalf", "低于此带 = 相对便宜,高于此带 = 相对偏贵(按官方成交四分位计算)"))}</small></div><div class="stat-card"><strong>${data.sample_size} ${escapeHtml(uiText("regionStats.officialRecords", "笔官方成交记录"))}</strong></div></div><p class="stats-exact">${escapeHtml(exact)}</p><p>${escapeHtml(uiText("regionStats.source", "出典: 不动产信息库（国土交通省） · 许可: PDL1.0"))}</p>${rentText}<p>${escapeHtml(uiText("regionStats.rentSource", "出典:政府統計の総合窓口(e-Stat)(https://www.e-stat.go.jp/)(加工して作成)"))}</p><p>${escapeHtml(uiText("regionStats.limitations", "限制：参考信息，非逐笔成交明细；㎡单价由官方总价和面积计算。"))}</p>`;
}
