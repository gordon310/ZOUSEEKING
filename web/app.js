...
function renderRegionStats() {
  ...
  const formatObservedMonth = (value) => {
    const match = String(value ?? "").match(/^(\d{4})-(0[1-9]|1[0-2])$/);
    if (!match) return String(value ?? "");
    const [, year, month] = match;
    return window.ZouI18n?.locale?.() === "en" ? `${month}/${year}` : `${year}年${Number(month)}月`;
  };
  ...
  const sourceNames = (data.sources || []).map((source) => source.name || source.url).filter(Boolean).join(" · ");
  const provenanceLine = [sourceNames || data.source_url, data.source_period, data.aggregation_method, data.missing_value_policy]
    .filter(Boolean).join(" · ");
